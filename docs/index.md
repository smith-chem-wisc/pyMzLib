...
!!! warning "Not on PyPI yet"
    pyMzLib has not had its first PyPI release. Until it does, download the wheel for your OS from
    the [latest GitHub release](https://github.com/smith-chem-wisc/pyMzLib/releases/latest) and
    `pip install` the file, or [build from source](contributing/building.md). The command above is
    what release day looks like, not what today looks like.
...
| [Readers](guides/readers.md) — read spectra from **mzML**, Thermo `.raw`, Bruker `.d`, timsTOF `.d`, MGF and msalign; identify any of 31 formats and read them, search results included | :material-check: available |
